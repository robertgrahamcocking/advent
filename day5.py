...
l = f.read().splitlines()
l = (list(
    map(lambda coord: coord.split(","), [i.replace(" -> ", ",") for i in l])))  # convert to list of four coordinates
...
def add_to_dict_and_count_2(l, coords):
    for i in l:
        ...
        xs = [j for j in range(x1, x2 + (-1 if x1 > x2 else 1), (-1 if x1 > x2 else 1))] if x1 != x2 else [x1 for _ in
                                                                                                           range(
                                                                                                               ylen + 1)]
        ys = [k for k in range(y1, y2 + (-1 if y1 > y2 else 1), (-1 if y1 > y2 else 1))] if y1 != y2 else [y1 for _ in
                                                                                                           range(
                                                                                                               xlen + 1)]
